...
class ValveLabel(DynamicLabel):
    def __init__(self, name: str, initial_state: str, row: int, column: int, parentGrid: QLayout):
        self.row = row
        self.column = column
    ...
    def update(self, new_state: packet_spec.ActuatorState):
        label_text = ""
        if (self.initial_state == "CLOSED" and new_state == packet_spec.ActuatorState.ON) \
            or (self.initial_state == "OPEN" and new_state == packet_spec.ActuatorState.OFF):
            label_text = "OPEN"
        else:
            label_text = "CLOSED"
    ...
    def update(self, new_state: packet_spec.ArmingState):
        match new_state:
            case packet_spec.ArmingState.ARMED_PAD:
                ...
                self.component.setStyleSheet("background-color: rgb(6, 171, 82); color: black;")
            case packet_spec.ArmingState.ARMED_VALVES:
                self.component.setText("2 - ARMED VALVES")
                self.component.setStyleSheet("background-color: rgb(174, 58, 239); color: white;")
            case packet_spec.ArmingState.ARMED_IGNITION:
                self.component.setText("3 - ARMED IGNITION")
                self.component.setStyleSheet("background-color: rgb(4, 110, 192); color: white;")
            case packet_spec.ArmingState.ARMED_DISCONNECTED:
                self.component.setText("4 - ARMED DISCONNECTED")
                self.component.setStyleSheet("background-color: rgb(252, 193, 0); color: black;")
            case packet_spec.ArmingState.ARMED_LAUNCH:
                self.component.setText("5 - ARMED LAUNCH")
                self.component.setStyleSheet("background-color: rgb(243, 5, 2); color: white;")
    ...
    def update(self, new_state: packet_spec.ContinuityState):
        match new_state:
            case packet_spec.ContinuityState.OPEN:
                ...
                self.component.setStyleSheet("background-color: rgb(255, 80, 80); color: black;")
